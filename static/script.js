...
function resetApplicationState() {
    ...
    window.companySetupState = {
        companyInfoCompleted: false,
        bankAccountCompleted: false,
        bothCompletedMessageShown: false
    };
...
function sendMessage() {
    ...
    // Bank account setup shortcut
    if (message.toLowerCase() === 'bank' || message.toLowerCase() === 'bank account' || message.toLowerCase() === 'banking') {
        input.value = '';
        addMessage(message, 'user'); // Show what user typed
        
        // Show bank account setup directly
        setTimeout(() => {
    ...
    // Handle tax configuration setup options (when user types numbered options)
    // Priority check: If we're in payroll context and user types numbers 1-4, treat as tax configuration
    const isNumberInput = /^[1-4]$/.test(message.trim());
    console.log('🔍 Number input check:', { isNumberInput, isInPayScheduleWorkflow: window.isInPayScheduleWorkflow, message: message.trim() });
    
    if (isNumberInput && window.isInPayScheduleWorkflow) {
        console.log('🎯 Tax configuration number detected:', message.trim());
        if (message.trim() === '1') {
            input.value = '';
        ...
        if (message.trim() === '4') {
            input.value = '';
            addMessage(message, 'user');
            handlePillClick('upload-tax-documents');
            return;
        }
    }
    ...
    // Handle Maine expansion scenario - when currentQuestion is 'business_expansion'
    if (isInTaxConfig && window.taxConfigurationState.currentQuestion === 'business_expansion') {
        const trimmedMessage = message.trim().toLowerCase();
        
        // Check for option 1: Add Maine taxes - handle various inputs
        if (trimmedMessage === '1' || 
            trimmedMessage === 'add' || 
            trimmedMessage.includes('add maine') || 
            trimmedMessage.includes('maine tax') ||
            trimmedMessage.includes('add tax') ||
            trimmedMessage.includes('maine')) {
            input.value = '';
            addMessage(message, 'user');
            handlePillClick('add-maine-taxes');
            return;
        }
        // Check for option 2: Upload documents
        else if (trimmedMessage === '2' || trimmedMessage.includes('upload') || trimmedMessage.includes('document') || trimmedMessage.includes('extraction')) {
            input.value = '';
            addMessage(message, 'user');
            handlePillClick('upload-documents-extraction');
            return;
        }
        // Check for option 3: Remote employees
        else if (trimmedMessage === '3' || trimmedMessage.includes('remote') || trimmedMessage.includes('other state')) {
            input.value = '';
            addMessage(message, 'user');
            handlePillClick('remote-employees-other-states');
            return;
        }
    }
    ...
    if (message.toLowerCase().trim() === 'ein') {
        ...
        // Use October 17, 2025 as the default launch date
        const defaultDate = new Date(2025, 9, 17); // October is month 9 (0-indexed)
        
        // Format date safely without timezone conversion
        const year = defaultDate.getFullYear();
        const month = String(defaultDate.getMonth() + 1).padStart(2, '0');
        const day = String(defaultDate.getDate()).padStart(2, '0');
        const safeStartDate = `${year}-${month}-${day}`;
    ...
    // Check for payroll transition options BEFORE welcome flow handler
    const messagesContainer = document.getElementById('chatMessages');
    const lastMessages = messagesContainer ? messagesContainer.innerText : '';
    const hasPayrollTransition = lastMessages.includes("What would you like to do next?") && 
                                 lastMessages.includes("1. Let's get started");
    
    // If we're at payroll transition screen, handle the options
    if (hasPayrollTransition) {
        const lowerMsg = message.toLowerCase().trim();
        
        // Option 1: Let's get started (various variations)
        if (lowerMsg === "1" || 
            lowerMsg === "1." ||
            lowerMsg.includes("let's get started") ||
            lowerMsg.includes("lets get started") ||
            lowerMsg.includes("let's start") ||
            lowerMsg.includes("lets start") ||
            lowerMsg === "get started" ||
            lowerMsg === "start") {
            input.value = '';
            addMessage(message, 'user');
            // Start pay schedule setup
            startPayScheduleSetup();
            return;
        }
        ...
        if (lowerMsg === "2" || 
            ...
            lowerMsg.includes("collaborator")) {
            input.value = '';
            addMessage(message, 'user');
            handlePillClick('invite-collaborator');
            return;
        }
        
        // Option 3: Save and done
        if (lowerMsg === "3" || 
            lowerMsg === "3." ||
            lowerMsg.includes("save") || 
            lowerMsg.includes("done for the day") ||
            lowerMsg.includes("done for today")) {
            input.value = '';
            addMessage(message, 'user');
            handlePillClick('save-and-done');
            return;
        }
    }
    
    // Now check welcome flow AFTER payroll transition check
    ...
    if (typeof content === 'object' && content.type === 'process-card') {
        ...
        messageDiv.innerHTML = `
            <div class="process-card" id="process-card-${content.id}">
                ...
                ${content.description ? `<div class="process-card-description">${content.description}</div>` : ''}
                <div class="process-card-meta">
                    <div class="process-card-status ${content.status}">
                        ${content.status === 'processing' ? '<div class="process-card-spinner"></div>' : '<span class="status-dot" style="width: 8px; height: 8px; background: #22c55e; border-radius: 50%; display: inline-block;"></span>'}
                        <span class="status-text">${content.status === 'processing' ? 'Processing...' : 'Complete'}</span>
                    </div>
    ...
    if ((responsePills && responsePills.length > 0) || (options.style === 'two-tier-interactive' && options.tierTwoOptions && options.tierTwoOptions.length > 0)) {
        ...
        if (options.style === 'two-tier-interactive' || options.style === 'two-tier-strategy') {
            ...
            if (options.sourceDocuments) {
                console.log('📄 Creating source documents HTML:', options.sourceDocuments);
                sourceDocsHtml = `
                        ...
                        <div class="source-docs-content" style="max-height: 0px; overflow: hidden; transition: max-height 0.3s ease;">
                            <div style="padding: 12px 0 0 24px; font-size: 12px; line-height: 1.6; color: #666;">
                                ${options.sourceDocuments.files.map(file => 
                                    `• <strong>${file.name}</strong>${file.description ? ` - ${file.description}` : ''}<br>`
                                ).join('')}
                            </div>
                        </div>
                    </div>
                ...
                console.log('📄 No source documents provided in options');
            }

            // Combine all tiers - source docs at the bottom, after both tier1 and tier2
            pillsHtml = tier1Html + tier2Html + sourceDocsHtml;
            console.log('📄 Final pills HTML structure order: tier1 buttons -> tier2 content -> source docs');
...
function addUnifiedMessage(content, sender, options = {}) {
    ...
    if (pills && pills.length > 0) {
        ...
        if (options.style === 'two-tier-interactive' || options.style === 'two-tier-strategy') {
            ...
            // TIER 2: List items (only if tierTwoOptions or relatedQuestions provided)
            const relatedItems = options.tierTwoOptions || options.relatedQuestions;
...
function handlePillClick(action) {
    ...
    if (action.startsWith('tax-') || action.includes('-tax-') ||
        ['start-tax-configuration', 'confirm-tax-types', 'modify-tax-types', 'add-missing-taxes', 
         ...
         'add-maine-taxes', 'upload-documents-extraction', 'remote-employees-other-states',
         ...
         'verify-state-registrations', 'state-tax-requirements', 'add-maine-tax', 'skip-maine-for-now',
         'continue-to-payment-schedules', 'check-additional-taxes', 'complete-tax-config',
         'no-additional-taxes', 'review-tax-summary', 'confirm-maine-taxes', 'abbreviations-confirmed', 
         'edit-abbreviations', 'configure-tax-rates', 'add-to-todo', 'upload-documents-now',
         'why-upload-docs', 'where-find-rate-letters', 'what-if-no-letter', 'use-standard-rates',
         'documents-uploaded', 'upload-later'].includes(action)) {
    ...
    // IMMEDIATELY hide ALL interactive elements in the clicked message (wizard buttons, pills, source docs)
    if (clickedPill) {
        const messageElement = clickedPill.closest('.message');
        if (messageElement) {
            // Hide wizard actions (contains the main buttons)
            const wizardActions = messageElement.querySelector('.wizard-actions');
            if (wizardActions) {
                wizardActions.remove(); // Remove completely instead of just hiding
            }
            
            // Hide tier 2 options box (related questions)
            const tier2Boxes = messageElement.querySelectorAll('div[style*="border: 1px solid #e0e0e0"]');
            tier2Boxes.forEach(box => box.remove());
            
            // Hide source documents
            const sourceDocs = messageElement.querySelector('.source-docs-section');
            if (sourceDocs) {
                sourceDocs.remove();
            }
            
            // Hide old-style pills
            const suggestedPills = messageElement.querySelector('.suggested-pills');
            if (suggestedPills) {
                suggestedPills.remove();
            }
        }
    }

    // Also remove old-style pills
    const allPills = document.querySelectorAll('.suggested-pills');
    allPills.forEach(pills => {
        if (pills.parentNode) {
            pills.parentNode.removeChild(pills);
    ...
    switch (action) {
            ...
            case 'timeline-looks-achievable':
                message = 'This looks achievable';
                addMessage(message, 'user');
                
                // SECOND CONVERSATION THREAD: Document upload
                setTimeout(() => {
                    const documentUploadText = `Awesome! Let's start by uploading some documents. We'll scan these to auto-fill information for you, saving time and reducing errors..`;
                    
                    addMessage(documentUploadText, 'ai', [
                        { action: 'upload-documents', text: 'Upload documents', buttonType: 'primary' },
                        { action: 'do-it-later', text: 'Do it later', buttonType: 'secondary' }
                    ], {
                        style: 'two-tier-interactive',
                        tierTwoOptions: [
            ...
            case 'what-if-miss-timeline':
                message = 'What if I miss the timeline?';
                addMessage(message, 'user');
                setTimeout(() => {
                    addMessage(`Don't worry about missing deadlines! Here's what you should know:

<strong>The timeline is flexible:</strong>
• Dates automatically adjust based on your progress
• No penalties for completing tasks early or late
• Focus on accuracy over speed

<strong>We'll help you stay on track:</strong>
• Automated reminders before key deadlines
• Real-time progress tracking
• Support available if you get stuck

The most important thing is ensuring everything is set up correctly for your first payroll run.`, 'ai');
                }, 1000);
                return;
                
            case 'set-reminder':
                message = 'Can I set a reminder?';
                addMessage(message, 'user');
                setTimeout(() => {
                    addMessage(`Yes, we'll automatically send you reminders! Here's how it works:

<strong>Automatic reminders:</strong>
• 7 days before each deadline
• 24 hours before critical tasks
...
You'll receive your first reminder about a week before your earliest task is due.`, 'ai');
                }, 1000);
                return;
            ...
            case 'vendor-paychex':
                ...
                return;
            case 'continue-company-setup':
                message = 'Continue Company Setup';
                addMessage(message, 'user');
                
                // Hide the timeline panel if visible
                const continueTablePanel = document.getElementById('tablePanel');
                const continueChatPanel = document.getElementById('chatPanel');
                
                if (continueTablePanel && continueTablePanel.classList.contains('visible')) {
                    // Animate panel sliding out to the right
                    if (typeof gsap !== 'undefined') {
                        gsap.to(continueTablePanel, {
                            x: '100%',
                            opacity: 0,
                            duration: 0.6,
                            ease: 'power2.inOut',
                            onComplete: () => {
                                continueTablePanel.style.display = 'none';
                                continueTablePanel.classList.remove('visible');
                                continueTablePanel.classList.add('hidden');
                                console.log('🔍 Timeline panel hidden');
                            }
                        });
                        
                        // Expand chat panel to centered width
                        if (continueChatPanel) {
                            gsap.to(continueChatPanel, {
                                width: '800px',
                                marginLeft: 'auto',
                                marginRight: 'auto',
                                duration: 0.6,
                                ease: 'power2.inOut',
                                onComplete: () => {
                                    continueChatPanel.classList.add('centered');
                                    continueChatPanel.classList.remove('right-panel-open');
                                    console.log('💬 Chat panel centered for company setup');
                                }
                            });
                        }
                    } else {
                        ...
                        if (continueChatPanel) {
                            continueChatPanel.style.width = '800px';
                            continueChatPanel.classList.add('centered');
                            continueChatPanel.classList.remove('right-panel-open');
                        }
                    }
                ...
                // Show company info after transition
                setTimeout(() => {
                    showCompanyInfoAfterThinking();
                    
                    // Re-enter company-info workflow
                    if (window.progressManager) {
            ...
            case 'confirm-company-info':
                ...
                // Initialize company setup state if needed
                if (!window.companySetupState) {
                    window.companySetupState = {
                        companyInfoCompleted: false,
                        bankAccountCompleted: false,
                        bothCompletedMessageShown: false
                ...
                window.companySetupState.companyInfoCompleted = true;
                console.log('✅ Company info completed. Bank account completed:', window.companySetupState.bankAccountCompleted);
                
                // Step 2: Progress Update with celebration (500ms delay)
                if (window.progressManager && !window.welcomeState) {
                    setTimeout(() => {
                        ...
                        // Update to complete pay schedule with celebration
                        if (window.progressManager.isActive && window.progressManager.currentSection === 'pay-schedule') {
                            window.progressManager.updateProgress(1); // Triggers rainbow celebration
                        }
                        console.log('✅ Completed pay schedule configuration with celebration');
                    }, 500);
                }
                
                // Step 3: Collapse conversation history (1 second delay)
                setTimeout(() => {
                    collapseConversationHistory("Company information configuration");
                    console.log('📋 Collapsed company information configuration threads');
                }, 1000);
                
                // Step 4: Create process card for company configuration (2 seconds delay)
                setTimeout(() => {
                    const processCard = createProcessCard({
                        title: 'Company configuration',
                        description: 'Configuring name, address and EIN...',
                        status: 'processing',
                    ...
                    console.log('📋 Created company configuration process card');
                }, 2000);
                ...
                setTimeout(() => {
                    ...
                    // Step 5: Display bank configuration interface (3 seconds delay)
                    showTypingIndicator();
                    
                    // Enter bank account workflow
                    if (window.progressManager) {
                        window.progressManager.enterWorkflow('bank-account', 0);
                    }
                    ...
                    setTimeout(() => {
                        ...
                        // Update the process card after 30 seconds
                        setTimeout(() => {
                            updateProcessCard('company-info', {
                                status: 'complete',
                                description: 'Company details successfully configured',
                                timestamp: '30 seconds ago'
                            });
                        }, 30000);
                        
                        return;
                        // Display the semi-monthly pay schedule in card format
                        const scheduleCardsHTML = `
                            <div class="schedule-cards-chat">
                                <div class="company-info-card schedule-card-chat">
                                    <div class="company-info-header">
                                        <h3 class="company-info-title">Semi-Monthly Payroll</h3>
                        ...
                        addMessage(`${CONSTANTS.MESSAGES.PERFECT} Company information confirmed! Now let's set up your pay schedule.\n\nI\'ve analyzed your pay registers and extracted information for your pay schedule:\n\n${scheduleCardsHTML}\n\nLet's verify this schedule first. Shall we start?`, 'ai', [
                            { action: 'looks-good', text: 'Looks good, continue', buttonType: 'primary' },
                            { action: 'add-new', text: 'Add new schedule', buttonType: 'secondary' },
                            { action: 'edit-semi-monthly', text: 'Edit Semi-Monthly', buttonType: 'secondary' }
                        ], {
            ...
            // === BANK ACCOUNT SETUP CASES ===
            case 'connect-bank':
                message = 'Connect to my bank';
                addMessage(message, 'user');
                ...
                setTimeout(() => {
                    ...
                    // Show bank connection interface
                    const bankConnectionHTML = `
                        <div class="bank-connection-container">
                            <h4>Select Your Bank</h4>
                            <p>Choose your financial institution to securely connect your account:</p>
                            <div class="bank-search">
                                <input type="text" placeholder="Search for your bank..." class="bank-search-input" onkeyup="filterBanks(this.value)">
                            </div>
                            <div class="bank-list">
                                <div class="bank-item" onclick="handlePillClick('select-bank-boa')">
                                    <div class="bank-logo">🏦</div>
                                    <span>Bank of America</span>
                                </div>
                                <div class="bank-item" onclick="handlePillClick('select-bank-chase')">
                                    <div class="bank-logo">🏦</div>
                                    <span>Chase</span>
                                </div>
                                <div class="bank-item" onclick="handlePillClick('select-bank-wells')">
                                    <div class="bank-logo">🏦</div>
                                    <span>Wells Fargo</span>
                                </div>
                                <div class="bank-item" onclick="handlePillClick('select-bank-citi')">
                                    <div class="bank-logo">🏦</div>
                                    <span>Citibank</span>
                                </div>
                                <div class="bank-item" onclick="handlePillClick('select-bank-other')">
                                    <div class="bank-logo">🏦</div>
                    ...
                    addUnifiedMessage(bankConnectionHTML, 'ai', {
                        pills: [
                            { action: 'manual-bank-entry', text: 'Enter details manually', buttonType: 'secondary' },
                            { action: 'skip-bank-setup', text: 'Skip for now', buttonType: 'secondary' }
                        ],
                        style: 'two-tier-interactive'
                    });
                }, 1000);
            ...
            case 'start-tax-setup':
            message = 'Start Tax Configuration';
            addMessage(message, 'user');
            
            showTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
                addMessage('Great! Let\'s start with your tax configuration. I\'ll guide you through setting up federal, state, and local taxes.', 'ai');
                
                // TODO: Add tax configuration interface
                setTimeout(() => {
                    addMessage('Tax configuration is in progress. After we complete this, we\'ll move on to earning codes and deductions.', 'ai', [
                        { action: 'continue-to-earning-codes', text: 'Continue to Earning Codes', buttonType: 'primary' }
                    ], {
                        style: 'two-tier-interactive'
                    });
                }, 2000);
        ...
        case 'skip-tax-setup':
            ...
            setTimeout(() => {
                removeTypingIndicator();
                addMessage('No problem! You can configure taxes later. Let\'s continue with earning codes and deductions.', 'ai');
                
                setTimeout(() => {
                    continueToEarningCodes();
                }, 1500);
            }, 1000);
            return;
            
        case 'continue-to-earning-codes':
            message = 'Continue to Earning Codes';
            addMessage(message, 'user');
            
            // Complete tax configuration workflow
            if (window.progressManager) {
                setTimeout(() => {
                    if (window.progressManager.currentSection === 'tax-configuration-simple') {
                        window.progressManager.updateProgress(3); // Complete tax setup
                    }
                }, 500);
            }
            
            // Continue to earning codes workflow
            setTimeout(() => {
                continueToEarningCodes();
            }, 1000);
            return;
        ...
        case 'lets-get-started':
            ...
            setTimeout(() => {
                removeTypingIndicator();
                addMessage('Perfect! Let\'s begin with configuring your pay schedules.', 'ai');
                
                // Start the actual pay schedule setup
                setTimeout(() => {
                    startPayScheduleSetup();
                }, 1000);
            }, 1000);
        ...
        case 'invite-collaborator':
            ...
            setTimeout(() => {
                removeTypingIndicator();
                addUnifiedMessage(`Invite a Team Member

To invite someone to help with setup, enter their email address below. They'll receive an invitation to collaborate on your payroll configuration.

<input type="email" placeholder="Enter email address" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px; margin: 16px 0;" id="collaborator-email">

The invited person will have access to:
- View and edit payroll configurations
- Add employee information
- Configure tax settings
- Set up earning codes and deductions`, 'ai', {
                    pills: [
                        { action: 'send-invitation', text: 'Send invitation', buttonType: 'primary' },
                        { action: 'lets-get-started', text: 'Skip and continue', buttonType: 'secondary' }
                    ],
                    style: 'two-tier-interactive'
                });
            }, 1000);
        ...
        case 'save-and-done':
            message = 'Save and done for the day';
            addMessage(message, 'user');
            
            showTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
                addMessage(`Great work today!

Your progress has been saved. Here's what we completed:
...
- Tax setup
- Earning codes and deductions
- Employee information

You can resume anytime by logging back in. Have a great rest of your day!`, 'ai', [
                    { action: 'continue-tomorrow', text: 'Remind me tomorrow', buttonType: 'primary' },
                    { action: 'continue-now', text: 'Actually, let me continue', buttonType: 'secondary' }
                ], {
                    style: 'two-tier-interactive'
                });
            }, 1000);
        ...
        case 'send-invitation':
            ...
            if (emailValue && emailValue.includes('@')) {
                ...
                setTimeout(() => {
                    removeTypingIndicator();
                    addMessage(`Invitation sent to ${emailValue}

They'll receive an email with instructions to join your team. In the meantime, let's continue with the configuration.`, 'ai');
                ...
                addMessage('Please enter a valid email address to send the invitation.', 'ai');
            }
            return;
        ...
        case 'continue-tomorrow':
            ...
            setTimeout(() => {
                removeTypingIndicator();
                addMessage('I\'ve set a reminder for tomorrow. You\'ll receive an email notification to continue your setup. See you then!', 'ai');
            }, 1000);
            return;
            
        case 'continue-now':
            message = 'Actually, let me continue';
            addMessage(message, 'user');
            
            showTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
                addMessage('Great! Let\'s keep the momentum going. Starting with pay schedule configuration now.', 'ai');
        ...
        case 'skip-bank-setup':
                message = 'Skip, do it later';
                addMessage(message, 'user');
                ...
                // Mark bank account as completed (even though skipped)
                window.companySetupState.bankAccountCompleted = true;
                console.log('✅ Bank account setup skipped. Company info completed:', window.companySetupState.companyInfoCompleted);
                
                showTypingIndicator();
                setTimeout(() => {
                    removeTypingIndicator();
                    addMessage('Understood! Just let me know when you want to set up your bank account.', 'ai');
                    
                    // Check if both company setup steps are complete
                    if (checkCompanySetupCompletion()) {
                        // Both are complete, continue to pay schedules after collapsing
                        setTimeout(() => {
                            continueToPaySchedules();
                        }, 1500);
                    } else {
                        // Just continue to pay schedules
                        setTimeout(() => {
                            continueToPaySchedules();
                        }, 1000);
                    }
            ...
            case 'bank-security':
                ...
                setTimeout(() => {
                    removeTypingIndicator();
                    addUnifiedMessage(`Yes, absolutely! Here's how we keep your information secure:

• **Bank-level encryption** - All data is encrypted using 256-bit SSL encryption
• **Read-only access** - We can only view account information, never move money
• **Secure authentication** - We use OAuth and never store your bank login credentials
• **SOC 2 compliant** - Our security practices are independently audited
• **PCI DSS certified** - We meet the highest standards for payment processing

Your bank credentials are never stored on our servers. We use trusted partners like Plaid to establish secure connections.`, 'ai', {
                        pills: [
                            { action: 'connect-bank', text: 'Connect to my bank', buttonType: 'primary' },
                            { action: 'skip-bank-setup', text: 'Skip for now', buttonType: 'secondary' }
                        ],
                        style: 'two-tier-interactive'
                    });
            ...
            case 'bank-requirements':
                ...
                setTimeout(() => {
                    removeTypingIndicator();
                    addUnifiedMessage(`To connect your bank account, you'll need:

• **Online banking credentials** - Username and password for your bank's website
• **Account selection** - Choose which checking account to use for payroll
• **Verification code** - Some banks may send a code to your phone or email

The whole process typically takes less than 2 minutes. You can also enter account details manually if you prefer.`, 'ai', {
                        pills: [
                            { action: 'connect-bank', text: 'Connect to my bank', buttonType: 'primary' },
                            { action: 'manual-bank-entry', text: 'Enter manually', buttonType: 'secondary' },
                            { action: 'skip-bank-setup', text: 'Skip for now', buttonType: 'secondary' }
                        ],
                        style: 'two-tier-interactive'
                    });
            ...
            case 'bank-change-later':
                message = 'Can I change banks later?';
                addMessage(message, 'user');
                
                showTypingIndicator();
                setTimeout(() => {
                    removeTypingIndicator();
                    addUnifiedMessage(`Yes, you can change your bank account at any time!

• **Easy switching** - Update your bank connection from Settings > Banking
• **No disruption** - Changes take effect for the next payroll run
• **Multiple accounts** - You can even use different accounts for different purposes
• **History preserved** - All your past transactions remain unchanged

There's no commitment - you're free to update your banking details whenever needed.`, 'ai', {
                        pills: [
                            { action: 'connect-bank', text: 'Connect to my bank', buttonType: 'primary' },
                            { action: 'skip-bank-setup', text: 'Set up later', buttonType: 'secondary' }
                        ],
                        style: 'two-tier-interactive'
                    });
                }, 1000);
            ...
            case 'bank-multiple':
                message = 'Can I use multiple accounts?';
                addMessage(message, 'user');
                
                showTypingIndicator();
                setTimeout(() => {
                    removeTypingIndicator();
                    addUnifiedMessage(`Yes! You can use multiple bank accounts for different purposes:

• **Primary payroll account** - For employee direct deposits
• **Tax account** - Separate account for tax withholdings
• **Benefits account** - For benefits and deductions
• **Operating account** - For payroll processing fees

Most businesses start with one account and add others as needed. You can always add more accounts later.`, 'ai', {
                        pills: [
                            { action: 'connect-bank', text: 'Connect first account', buttonType: 'primary' },
                            { action: 'skip-bank-setup', text: 'Set up later', buttonType: 'secondary' }
                        ],
                        style: 'two-tier-interactive'
                    });
            ...
            case 'select-bank-boa':
            case 'select-bank-chase':
            case 'select-bank-wells':
            case 'select-bank-citi':
            case 'select-bank-other':
                const bankNames = {
                    'select-bank-boa': 'Bank of America',
                    'select-bank-chase': 'Chase',
                    'select-bank-wells': 'Wells Fargo',
                    'select-bank-citi': 'Citibank',
                    'select-bank-other': 'Other Bank'
                };
                message = bankNames[action];
                addMessage(message, 'user');
                ...
                setTimeout(() => {
                    removeTypingIndicator();
                    addMessage(`Connecting to ${bankNames[action]}...\n\n✅ Bank account successfully connected!\n\nYour ${bankNames[action]} account has been securely linked for payroll processing.`, 'ai');
                    
                    // Mark bank account as completed
                    window.companySetupState.bankAccountCompleted = true;
                    console.log('✅ Bank account connected. Company info completed:', window.companySetupState.companyInfoCompleted);
                    
                    // Trigger celebration for bank account completion
                    if (window.progressManager && window.progressManager.renderers && window.progressManager.renderers.floating) {
                        setTimeout(() => {
                            window.progressManager.renderers.floating.celebrate('professional');
                        }, 500);
                    }
                    ...
                    if (checkCompanySetupCompletion()) {
                        // Both are complete, continue to pay schedules after collapsing
                        setTimeout(() => {
                            addMessage('Perfect! Company setup is complete. Let\'s continue with your pay schedule setup.', 'ai');
                            setTimeout(() => {
                                continueToPaySchedules();
                            }, 1000);
                        }, 1500);
                        ...
                        // Just continue to pay schedules
                        setTimeout(() => {
                            addMessage('Great! Now let\'s set up your pay schedules.', 'ai');
                            setTimeout(() => {
                                continueToPaySchedules();
                            }, 1000);
                        }, 1000);
            ...
            case 'manual-bank-entry':
                message = 'Enter details manually';
                addMessage(message, 'user');
                ...
                setTimeout(() => {
                    removeTypingIndicator();
                    addMessage('Please enter your bank account details:\n\n**Routing Number:** [Enter 9-digit routing number]\n**Account Number:** [Enter account number]\n**Account Type:** Checking / Savings\n\n(For demo purposes, we\'ll simulate successful entry)', 'ai');
                    
                    // Simulate successful manual entry after a delay
                    setTimeout(() => {
                        addMessage('✅ Bank account details saved successfully!', 'ai');
                        
                        // Mark bank account as completed
                        window.companySetupState.bankAccountCompleted = true;
                        console.log('✅ Bank account manually entered. Company info completed:', window.companySetupState.companyInfoCompleted);
                        ...
                        if (checkCompanySetupCompletion()) {
                            // Both are complete, continue to pay schedules after collapsing
                            setTimeout(() => {
                                addMessage('Excellent! Company setup is complete. Let\'s continue with your pay schedule setup.', 'ai');
                                setTimeout(() => {
                                    continueToPaySchedules();
                                }, 1000);
                            }, 1500);
                            ...
                            // Just continue to pay schedules
                            setTimeout(() => {
                                addMessage('Now let\'s set up your pay schedules.', 'ai');
                                setTimeout(() => {
                                    continueToPaySchedules();
                                }, 1000);
                            }, 1000);
            ...
            setTimeout(() => {
                ...
                addMessage(`${CONSTANTS.MESSAGES.PERFECT} I\'ve analyzed your pay registers and extracted information for your pay schedule:\n\n<strong>Semi-Monthly Payroll</strong>\n- Name: Semi-Monthly Payroll\n- First pay date: August 15, 2025\n- Pay frequency: 24 pay periods/year\n- Pay dates: 15th and last day of each month\n- Hours per pay period: 80 hours\n- Weekend pay date: Friday before the date\n- Holiday pay date: Business before the date\n\nLet's verify this schedule first. Shall we start?`, 'ai', [
                    ...
                    sourceDocuments: {
                        count: 2,
                        files: [
                            { name: 'pay_registers.csv', description: 'Historical payroll data with dates and amounts' },
                            { name: 'schedule_config.json', description: 'Extracted pay schedule configurations' }
                        ]
                    }
                });
            }, 1000);
            ...
            case 'looks-good':
                ...
                // Show tax configuration setup after process card
                setTimeout(() => {
                    // Call the tax configuration workflow directly to show federal tax overview
                    if (typeof startTaxConfigurationWorkflow === 'function') {
                        startTaxConfigurationWorkflow();
                    } else {
                        console.error('startTaxConfigurationWorkflow is not defined');
                    }
                }, 3000);
                return;
...
Let's verify this schedule first. Shall we start?`, 'ai', [
    { action: 'continue-earning-codes', text: 'Looks good, continue', buttonType: 'primary' },
    { action: 'add-new', text: 'Add new schedule', buttonType: 'secondary' },
    { action: 'edit-semi-monthly', text: 'Edit Semi-Monthly', buttonType: 'secondary' }
], {
    style: 'two-tier-interactive'
...
Please review this information and confirm it's correct.`;
        ...
        case 'edit-done': {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    addMessage(`${CONSTANTS.MESSAGES.PERFECT} Company information updated! Now let's set up your pay schedule.\n\nI've analyzed your pay registers and extracted information for your pay schedule:\n\n${scheduleCardsHTML}\n\nLet's verify this schedule first. Shall we start?`, 'ai', [
                        { action: 'looks-good', text: 'Looks good, continue', buttonType: 'primary' },
                        { action: 'add-new', text: 'Add new schedule', buttonType: 'secondary' },
                        { action: 'edit-semi-monthly', text: 'Edit Semi-Monthly', buttonType: 'secondary' }
                    ], {
            ...
            case 'do-it-later':
                ...
                setTimeout(() => {
                    addMessage('No problem! You can upload documents anytime during setup. I\'ll guide you through manual configuration for now.\n\nReady to continue with your timeline?', 'ai', [
                          { action: 'start-schedule-setup', text: 'Skip to payroll configuration', buttonType: 'secondary' },
    ...
    style: 'two-tier-interactive'
        ...
        case 'skip-to-company-config':
            ...
            setTimeout(() => {
                // Transition to existing payroll setup
                loadInitialScheduleCards();
              addMessage('No problem! I\'ll guide you through manual configuration.\n\nI\'ve analyzed your pay registers and extracted information for your pay schedule. Let\'s verify this schedule first.', 'ai', [
    { action: 'looks-good', text: 'Looks good, continue', buttonType: 'primary' },
    { action: 'add-new', text: 'Add new schedule', buttonType: 'secondary' },
    { action: 'edit-semi-monthly', text: 'Edit Semi-Monthly', buttonType: 'secondary' }
], {
    style: 'two-tier-interactive'
        ...
        case 'edit-schedule':
            ...
            response = {
                text: 'I can help you modify your Semi-Monthly schedule.',
                pills: [
...
function calculateEarningCodesStats() {
    ...
    return {
        ...
        case 'continue-tax-configuration':
        addMessage('Continue to Tax Configuration', 'user');
        ...
        // Set flag to indicate we're in tax configuration workflow
        window.isInTaxConfigWorkflow = true;
        
        // Show tax configuration setup
        setTimeout(() => {
            // Show tax configuration interface
            showTaxConfigurationSetup();
        }, 3000);
    ...
    switch (action) {
        ...
        case 'general-clarification':
            ...
            clarificationPills = [
                ...
                { action: 'add-employee', text: 'Set up employee' }
            ];
            break;
...
function createProcessCard(config) {
    ...
    // If duration is specified and status is processing, auto-complete after duration
    if (config.duration && config.status === 'processing') {
        setTimeout(() => {
            const cardElement = document.getElementById(`process-card-${card.id}`);
            if (cardElement) {
                const statusElement = cardElement.querySelector('.process-card-status');
                if (statusElement) {
                    statusElement.classList.remove('processing');
                    statusElement.classList.add('complete');
                    statusElement.innerHTML = '<span class="status-dot" style="width: 8px; height: 8px; background: #22c55e; border-radius: 50%; display: inline-block;"></span><span class="status-text">Complete</span>';
                }
            }
        }, config.duration);
    }
...
function showEarningCodesReview(options = {}) {
    ...
    addMessage(getEarningCodeIntroMessage(true), 'ai', [
        ...
        { action: 'skip-to-other-setup', text: 'Skip to other payroll configuration', buttonType: 'secondary' }
    ], {
        style: 'two-tier-interactive',
        tierTwoOptions: [
...
function recalculateTimelineDates(newLaunchDate) {
    ...
    const dateParts = newLaunchDate.split('-');
    const launchDate = new Date(parseInt(dateParts[0]), parseInt(dateParts[1]) - 1, parseInt(dateParts[2]));

    // Company configuration tasks (work backwards from launch date using milliseconds)
    const companyInfoDue = new Date(launchDate.getTime() - (45 * 24 * 60 * 60 * 1000)); // 45 days before launch
    const bankAccountDue = new Date(launchDate.getTime() - (40 * 24 * 60 * 60 * 1000)); // 40 days before launch
    const employeeInfoDue = new Date(launchDate.getTime() - (35 * 24 * 60 * 60 * 1000)); // 35 days before launch

    // Payroll setup tasks
    const payScheduleDue = new Date(launchDate.getTime() - (30 * 24 * 60 * 60 * 1000)); // 30 days before launch
    const earningDeductionDue = new Date(launchDate.getTime() - (20 * 24 * 60 * 60 * 1000)); // 20 days before launch
    const taxSetupDue = new Date(launchDate.getTime() - (10 * 24 * 60 * 60 * 1000)); // 10 days before launch
...
function calculateTimelineDates() {
    ...
    if (window.newWizardState && window.newWizardState.userData && window.newWizardState.userData.startDate) {
        ...
        const startDateString = window.newWizardState.userData.startDate;
        const dateParts = startDateString.split('-');
        launchDate = new Date(parseInt(dateParts[0]), parseInt(dateParts[1]) - 1, parseInt(dateParts[2]));
        console.log('📅 Using startDate from newWizardState:', startDateString);
    } else {
        // Default to October 17, 2025
        launchDate = new Date(2025, 9, 17); // Month is 0-indexed, so 9 = October
        console.log('📅 Using default launch date: October 17, 2025');
    }

    // Create dates by adding/subtracting days to avoid timezone issues
    const companyInfoDue = new Date(launchDate.getTime() - (50 * 24 * 60 * 60 * 1000)); // 50 days before launch
    const bankAccountDue = new Date(launchDate.getTime() - (45 * 24 * 60 * 60 * 1000)); // 45 days before launch
    
    // Reordered to match new workflow sequence
    const payScheduleDue = new Date(launchDate.getTime() - (40 * 24 * 60 * 60 * 1000)); // 40 days before launch (moved earlier)
    const taxSetupDue = new Date(launchDate.getTime() - (30 * 24 * 60 * 60 * 1000)); // 30 days before launch (moved earlier)
    const earningDeductionDue = new Date(launchDate.getTime() - (20 * 24 * 60 * 60 * 1000)); // 20 days before launch
    const employeeInfoDue = new Date(launchDate.getTime() - (10 * 24 * 60 * 60 * 1000)); // 10 days before launch (moved later)
                    ...
                    <div class="timeline-section">
                        <div class="timeline-items">
                            ...
                            <div class="timeline-item">
                                ...
                                <div class="timeline-task-content">
                                    <div class="timeline-task-info">
                                        <div class="timeline-task">Connect bank account</div>
                    ...
                    <div class="timeline-section">
                        ...
                        <div class="timeline-items">
                            <div class="timeline-item">
                                ...
                                <div class="timeline-task-content">
                                    <div class="timeline-task-info">
                                        <div class="timeline-task">Set up pay schedule</div>
                            ...
                            <div class="timeline-item">
                                ...
                                <div class="timeline-task-content">
                                    <div class="timeline-task-info">
                                        <div class="timeline-task">Configure tax</div>
                            ...
                            <div class="timeline-item">
                                ...
                                <div class="timeline-task-content">
                                    <div class="timeline-task-info">
                                        <div class="timeline-task">Add earning and deduction code</div>
                            ...
                            <div class="timeline-item">
                                ...
                                <div class="timeline-task-content">
                                    <div class="timeline-task-info">
                                        <div class="timeline-task">Create employee information</div>
    ...
    if (panelActions) {
        panelActions.innerHTML = `
            <button class="next-step-btn" onclick="completePayrollSetup()">
                🎉 Complete Payroll configuration
            </button>
        `;
    }
}
...
function showTimelineWelcomeMessage() {
    ...
    // FIRST CONVERSATION THREAD: Timeline confirmation
    const timelineConfirmText = `I've created your personalized setup timeline based on your ${selectedDateFormatted} launch date.

<strong>How does this timeline look?</strong>`;
    
    removeTypingIndicator();
    addMessage(timelineConfirmText, 'ai', [
        { action: 'timeline-looks-achievable', text: 'This looks achievable', buttonType: 'primary' },
        { action: 'modify-timeline', text: 'I need to modify dates', buttonType: 'secondary' }
    ], {
        style: 'two-tier-interactive',
        tierTwoOptions: [
            { action: 'what-if-miss-timeline', text: 'What if I miss the timeline?' },
            { action: 'set-reminder', text: 'Can I set a reminder?' }
        ]
    });
}
...
function simulateDocumentProcessing(hasHandbook, hasPayroll) {
    // PHASE 1: Start processing immediately with spinner
    setTimeout(() => {
        // Show extraction spinner when progress bars start animating
        addMessage(`<div class="extraction-spinner-container" style="text-align: center; padding: 20px;">
            <div class="extraction-spinner" style="display: inline-block; width: 50px; height: 50px; border: 3px solid #f3f3f3; border-top: 3px solid #3498db; border-radius: 50%; animation: spin 1s linear infinite;"></div>
            <div style="margin-top: 15px; color: #666; font-size: 14px;">Extracting company information from documents...</div>
        </div>
        <style>
        @keyframes spin {
        ...
        // PHASE 1.5: Show progress summary with confetti after 4 seconds
        setTimeout(() => {
            // Remove the spinner message
            const messages = document.querySelectorAll('.message');
            for (let i = messages.length - 1; i >= 0; i--) {
                if (messages[i].querySelector('.extraction-spinner-container')) {
                    messages[i].remove();
                    break;
                }
            }
            ...
            addMessage('🎉 Great progress!\n\nBased on your uploaded documents, here\'s what we\'ve completed:\n\nCompany configuration:\n 45% complete\n\nPayroll configuration:\n 43% complete\n\n<strong>Next: Let\'s finish your company setup.</strong>', 'ai', [
...
function animateImplementationProgress() {
    ...
    const progressData = [
        { progress: 90, status: null },  // Company info
        { progress: 45, status: null },  // Bank account
        { progress: 90, status: null },  // Pay schedule
        { progress: 20, status: null },  // Tax setup
        { progress: 20, status: null },  // Earning and deduction
        { progress: 80, status: null }   // Employee info
    ];
...
function showBankAccountSetup() {
    const bankSetupMessage = `<strong>Bank Account Setup</strong>

We need to set up your default company bank account to debit for wages, taxes, and your monthly UKG fee. This will be your primary account for all payroll transactions.

<strong>Would you like to connect your bank account now or set it up later?</strong>`;
    
    addUnifiedMessage(bankSetupMessage, 'ai', {
        pills: [
            { action: 'connect-bank', text: 'Connect to my bank', buttonType: 'primary' },
            { action: 'skip-bank-setup', text: 'Skip, do it later', buttonType: 'secondary' }
        ],
        style: 'two-tier-interactive',
        relatedQuestions: [
            { action: 'bank-security', text: 'Is my banking information secure?' },
            { action: 'bank-change-later', text: 'Can I change bank accounts later?' },
            { action: 'bank-multiple', text: 'Can I use different accounts for taxes and wages?' }
        ]
    });
}

// Tax Configuration Setup Workflow
function showTaxConfigurationSetup() {
    const taxSetupMessage = `<strong>Time to configure your taxes</strong>
Our goal is to have everything ready by August 27, 2025. That gives us plenty of time to get it right before you run your first payroll.

<strong>We'll need to:</strong>
• Configure state tax requirements
• Determine deposit frequency
• Customize how employees see taxes on their pay stubs

<strong>How would you like to proceed?</strong>

1. Start tax configuration
2. Invite a coworker to handle this section
3. Skip taxes for now and continue with other configurations
4. Upload tax documents`;
    
    addMessage(taxSetupMessage, 'ai');
    
    // Set flag to enable numbered input detection for tax configuration
    window.isInPayScheduleWorkflow = true;
    console.log('🔧 Tax configuration setup displayed - set isInPayScheduleWorkflow flag to enable numbered navigation');
    
    // Enter tax configuration workflow in progress manager
    if (window.progressManager) {
        setTimeout(() => {
            window.progressManager.enterWorkflow('tax-configuration-simple', 0);
        }, 500);
    }
}

// Continue to earning codes from tax configuration
function continueToEarningCodes() {
    // Enter earning codes workflow
    if (window.progressManager) {
        window.progressManager.enterWorkflow('earnings-code', 0);
    }
    ...
    setTimeout(() => {
        removeTypingIndicator();
        addMessage('Now let\'s set up your earning codes and deductions. This will define how your employees are compensated.', 'ai');
        
        // Show earning codes interface
        setTimeout(() => {
            showEarningCodesReview();
        }, 1500);
    }, 1000);
...
// Continue to pay schedules from bank setup
function continueToPaySchedules() {
    showTypingIndicator();
    
    setTimeout(() => {
        removeTypingIndicator();
        
        // Show transition screen to payroll setup
        showPayrollSetupTransition();
    }, 1000);
}

// Show payroll setup transition with steps and deadlines
function showPayrollSetupTransition() {
    // Get the actual timeline dates we already calculated
    let launchDate;
    ...
    // Employee info is the last step - 10 days before launch
    const employeeInfoDue = new Date(launchDate.getTime() - (10 * 24 * 60 * 60 * 1000));
    const deadlineStr = employeeInfoDue.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    
    // Calculate all timeline dates to match what's shown in timeline
    const timelineDates = calculateTimelineDates();
    
    const transitionMessage = `<strong>Let's finish setting up your payroll.</strong> 
    
    Our goal is to have everything ready by ${deadlineStr}. That gives us plenty of time to get it right before you run your first payroll.

<strong>We'll set up:</strong>
1. Pay Schedules (due ${timelineDates.paySchedule})
2. Tax Configuration (due ${timelineDates.taxSetup})
3. Earnings & Deductions (due ${timelineDates.earningDeduction}) 
4. Employee Information (due ${timelineDates.employeeInfo}) 

Each step takes about 10-15 minutes. You can pause anytime and resume where you left off.

<strong>What would you like to do next?</strong>
1. <strong>Let's get started</strong> with the payroll schedule setup
2. <strong>Invite a coworker</strong> to handle this section  
3. <strong>Skip payroll configuration</strong> for now and continue with other configurations`;
    
    addUnifiedMessage(transitionMessage, 'ai');
}

// Original continueToPaySchedules content - now called after transition
function startPayScheduleSetup() {
    // Enter pay schedule workflow
    if (window.progressManager) {
        window.progressManager.enterWorkflow('pay-schedule', 0);
    }
    ...
    setTimeout(() => {
        ...
        const messageContent = `<strong>Pay Schedule Setup</strong>
<div style="margin-top: 2px; margin-bottom: 0;">I've extracted your pay schedule from your documents. Please review it:</div>
${scheduleCardsHTML}`;

        const sourceDocsHTML = `
<div style="margin-top: 4px;">
    <div style="display: flex; align-items: center; cursor: pointer; padding: 4px 0; color: #666; font-size: 14px;" onclick="this.nextElementSibling.style.display = this.nextElementSibling.style.display === 'none' ? 'block' : 'none'; this.querySelector('.expand-icon').textContent = this.nextElementSibling.style.display === 'none' ? '▶' : '▼';">
        <span class="expand-icon" style="margin-right: 8px; font-size: 12px;">▶</span>
        <span style="margin-right: 4px;">📄</span>
        <span>Source Documents (1)</span>
    </div>
    <div style="display: none; margin-left: 28px; margin-top: 8px;">
        <div style="padding: 4px 0; color: #555; font-size: 13px;">• Employee_Payroll_Report_2025.pdf</div>
    </div>
</div>`;

        addUnifiedMessage(messageContent, 'ai', {
            pills: [
                { action: 'looks-good', text: 'Looks good, continue', buttonType: 'primary' },
                { action: 'edit-semi-monthly', text: 'Edit Semi-Monthly', buttonType: 'secondary' },
                { action: 'add-new', text: 'Add new schedule', buttonType: 'secondary' }
...
function showCompanyInfoAfterThinking() {
    ...
    const confirmationText = `Excellent progress! We've got the foundation pieces from your uploaded documents:

Legal company name and address ✓

Federal EIN ✓

${companyCardHtml}<strong>Is this information extracted correctly?</strong>`;
    
    console.log("📝 Adding message with company info");
    addMessage(confirmationText, 'ai', [
        { action: 'confirm-company-info', text: 'Yes, proceed with configuration', buttonType: 'primary' },
        { action: 'make-corrections', text: 'Edit info', buttonType: 'secondary' },
        { action: 'add-new-ein', text: 'Add new company', buttonType: 'secondary' }
    ], {
        style: 'two-tier-interactive',
...
// Function to go back to implementation timeline
function backToImplementationTimeline() {
    console.log("🔙 Back to Implementation timeline clicked");
    
    // Check if right panel exists, if not create it
    let tablePanel = document.getElementById('tablePanel');
    const chatPanel = document.getElementById('chatPanel');
    
    if (!tablePanel) {
        // Create the right panel if it doesn't exist
        tablePanel = document.createElement('div');
        tablePanel.id = 'tablePanel';
        tablePanel.className = 'table-panel hidden';
        ...
        `;
        document.querySelector('.app-container').appendChild(tablePanel);
    }
    
    // Show the right panel with animation
    if (typeof gsap !== 'undefined') {
        ...
        gsap.fromTo(tablePanel, 
            ...
            },
            {
                ...
                ease: 'power2.inOut',
                onComplete: () => {
                    console.log('📊 Timeline panel slide-in complete');
                }
            }
        );
        
        // Shrink chat panel back to original width
        if (chatPanel) {
            gsap.to(chatPanel, {
                width: '30%',
                marginLeft: '0',
                ...
                onComplete: () => {
                    ...
                    console.log('💬 Chat panel resized for timeline view');
                }
            });
        }
    } else {
    ...
    // Load the timeline content with progress
    setTimeout(() => {
        const dates = calculateTimelineDates();
        loadTimelineWithProgress(dates);
    }, 300);
    
    // Hide the company info message and show timeline message
    setTimeout(() => {
        addMessage("Welcome back to your Implementation Timeline! Here you can see your progress across all setup tasks.", 'ai', [
            { action: 'continue-company-setup', text: 'Continue Company Setup', buttonType: 'primary' },
            { action: 'modify-timeline', text: 'Modify timeline dates', buttonType: 'secondary' }
        ]);
    }, 700);
}

// Function to load timeline with progress (similar to existing implementation)
function loadTimelineWithProgress(dates) {
    const panelContent = document.querySelector('.panel-content');
    
    if (panelContent) {
        // Use the existing timeline HTML structure with animated progress bars
        panelContent.innerHTML = `
            <div class="timeline-view-container">
                <div class="timeline-section">
                    <div class="section-header">
                        ...
                        <div class="section-progress">
                            <div class="section-progress-wrapper">
                                <div class="section-progress-bar" data-section="company" style="width: 45%; background: #4CAF50; transition: all 0.8s ease;"></div>
                            </div>
                            <span class="section-progress-text">45%</span>
                        </div>
                    </div>
                    <div class="timeline-items">
                        <div class="timeline-item">
                            ...
                            <div class="timeline-task-content">
                                <div class="timeline-task-info">
                                    <div class="timeline-task">Add company information</div>
                                    <div class="timeline-due">Due by ${dates.companyInfo}</div>
                                </div>
                                <div class="timeline-progress">
                                    <div class="timeline-progress-bar" style="width: 90%; background: #4CAF50;"></div>
                                </div>
                            </div>
                        </div>
                        <div class="timeline-item">
                            ...
                            <div class="timeline-task-content">
                                <div class="timeline-task-info">
                                    <div class="timeline-task">Connect bank account</div>
                                    <div class="timeline-due">Due by ${dates.bankAccount}</div>
                                </div>
                                <div class="timeline-progress">
                                    <div class="timeline-progress-bar" style="width: 0%; background: #e0e0e0;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="timeline-section">
                    <div class="section-header">
                        ...
                        <div class="section-progress">
                            <div class="section-progress-wrapper">
                                <div class="section-progress-bar" data-section="payroll" style="width: 43%; background: #4CAF50; transition: all 0.8s ease;"></div>
                            </div>
                            <span class="section-progress-text">43%</span>
                        </div>
                    </div>
                    <div class="timeline-items">
                        <div class="timeline-item">
                            ...
                            <div class="timeline-task-content">
                                <div class="timeline-task-info">
                                    <div class="timeline-task">Employee information upload</div>
                                    <div class="timeline-due">Due by ${dates.employeeInfo}</div>
                                </div>
                                <div class="timeline-progress">
                                    <div class="timeline-progress-bar" style="width: 90%; background: #4CAF50;"></div>
                                </div>
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-task-content">
                                <div class="timeline-task-info">
                                    <div class="timeline-task">Set up pay schedule</div>
                                    <div class="timeline-due">Due by ${dates.paySchedule}</div>
                                </div>
                                <div class="timeline-progress">
                                    <div class="timeline-progress-bar" style="width: 20%; background: #FFA726;"></div>
                                </div>
                            </div>
                        </div>
                        <div class="timeline-item">
                            ...
                            <div class="timeline-task-content">
                                <div class="timeline-task-info">
                                    <div class="timeline-task">Earning and deduction codes</div>
                                    <div class="timeline-due">Due by ${dates.earningDeduction}</div>
                                </div>
                                <div class="timeline-progress">
                                    <div class="timeline-progress-bar" style="width: 20%; background: #FFA726;"></div>
                                </div>
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-task-content">
                                <div class="timeline-task-info">
                                    <div class="timeline-task">Configure tax</div>
                                    <div class="timeline-due">Due by ${dates.taxSetup}</div>
                                </div>
                                <div class="timeline-progress">
                                    <div class="timeline-progress-bar" style="width: 80%; background: #4CAF50;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        console.log('📊 Timeline loaded with progress bars');
    }
}
                ...
                panelContent.innerHTML = `
                    <div class="panel-content-container">
                        <div class="step-progress">
                            📋 Ready to continue with payroll configuration
                        </div>
                        <p style="color: var(--text-secondary); padding: var(--spacing-lg);">
                            Company information editing completed. You can continue with the next step in your payroll configuration.
                        </p>
                    </div>
                `;
...
// Check if both company setup steps are complete
function checkCompanySetupCompletion() {
    if (!window.companySetupState) {
        window.companySetupState = {
            companyInfoCompleted: false,
            bankAccountCompleted: false,
    ...
    if (window.companySetupState.companyInfoCompleted && 
        window.companySetupState.bankAccountCompleted && 
        !window.companySetupState.bothCompletedMessageShown) {
        
        window.companySetupState.bothCompletedMessageShown = true;
        
        // Don't collapse - just update progress
        // Update progress to show company setup is complete
        if (window.progressManager) {
            // Complete the company-info workflow (which includes bank account)
            if (window.progressManager.currentSection === 'company-info') {
                window.progressManager.updateProgress(2); // Complete both substeps
            }
        }
        
        console.log('✅ Company Setup fully completed (both company info and bank account)');
...
document.addEventListener('keydown', function(event) {
    ...
    // Add keyboard shortcut for bank account setup - Ctrl+B
    if (event.ctrlKey && event.key === 'b') {
        event.preventDefault();
        console.log('Keyboard shortcut: Jumping to bank account setup');
        
        // Clear any existing messages and show bank setup directly
        const messagesContainer = document.getElementById('chatMessages');
        if (messagesContainer) {
            // Add a system message about the shortcut
            addMessage('Jumping to bank account setup...', 'system');
        }
        
        // Show visual feedback
        const indicator = document.createElement('div');
        indicator.style.cssText = 'position:fixed;top:20px;right:20px;background:#22c55e;color:white;padding:8px 12px;border-radius:4px;z-index:9999;';
        indicator.textContent = 'Bank Account Setup (Ctrl+B)';
        document.body.appendChild(indicator);
        setTimeout(() => indicator.remove(), 2000);
        
        // Call the existing bank account setup function
        setTimeout(() => {
            showBankAccountSetup();
        }, 500);
    }
...
function handlePrototypeUpload() {
    ...
    setTimeout(() => {
        ...
        addMessage('<strong>Uploaded Files:</strong>\n<div class="uploaded-files-list">\n<div class="uploaded-file" data-filename="Employee_Handbook_2024.pdf">\n  <span class="file-name">Employee_Handbook_2024.pdf (2.3 MB)</span>\n  <span class="file-action trash-icon" onclick="toggleFileUpload(\'Employee_Handbook_2024.pdf\', this)">🗑️</span>\n</div>\n<div class="uploaded-file" data-filename="Payroll_Register_JantoJun2025.xlsx">\n  <span class="file-name">Payroll_Register_JantoJun2025.xlsx (1.1 MB)</span>\n  <span class="file-action trash-icon" onclick="toggleFileUpload(\'Payroll_Register_JantoJun2025.xlsx\', this)">🗑️</span>\n</div>\n<div class="uploaded-file" data-filename="Paycom_Quarterly_Payroll_Register_Q12025.xlsx">\n  <span class="file-name">Paycom Quarterly Payroll Register_Q12025.xlsx (1.4 MB)</span>\n  <span class="file-action trash-icon" onclick="toggleFileUpload(\'Paycom_Quarterly_Payroll_Register_Q12025.xlsx\', this)">🗑️</span>\n</div>\n<div class="uploaded-file" data-filename="Employee_Information_Report_2025.xlsx">\n  <span class="file-name">Employee Information Report 2025.xlsx (892 KB)</span>\n  <span class="file-action trash-icon" onclick="toggleFileUpload(\'Employee_Information_Report_2025.xlsx\', this)">🗑️</span>\n</div>\n</div>\n\nGot it! I can see exactly what I need in these files\n\n<strong>Should I go ahead and process them?</strong>', 'ai', [
            { action: 'confirm-uploaded-files', text: 'Yes, process these files', buttonType: 'primary' },
            { action: 'upload-different-files', text: 'Upload more files', buttonType: 'secondary' }
        ], {
            style: 'two-tier-interactive'
        });
    }, 1500);
