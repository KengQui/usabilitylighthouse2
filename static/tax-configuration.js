...
function handleTaxConfigurationPill(action) {
    ...
    switch (action) {
        ...
        case 'maine-tax-penalties':
            ...
            case 'confirm-payment-schedules':
                return confirmPaymentSchedules();
            case 'modify-payment-schedules':
...
function startTaxConfigurationWorkflow() {
    ...
    addMessage(`<strong>Tax configuration</strong>

Good progress! 🎉 Next, let's set up your tax configuration. We'll walk you through your tax setup step by step. UKG will handle all the calculations and payments automatically once configured.
...
<strong>1. Set up</strong> state tax configuration for your specific states
<strong>2. Invite a coworker</strong> to handle this section
<strong>3. Skip taxes</strong> for now and continue with other configurations
<strong>4. Ask more</strong> clarification questions`, 'ai', [], {
        style: 'two-tier-interactive',
        tierTwoOptions: [
            ...
            { action: 'additional-tax-info', text: 'What additional information would I need to provide for tax set up' }
        ]
    });
...
Great news! UKG will automatically set up and manage your federal taxes for you. These are required for all employers and will be configured with the proper rates and payment schedules.
...
- Apply same payment frequency to similar taxes
...
Review and adjust payment frequencies for current batch:
...
❓ <strong>Migration Question:</strong> Did your previous vendor automatically handle the employer portion of payroll taxes, or do you have those account details stored separately?`;
        ...
        case 'business_expansion':
            ...
            if (expansionStates.includes('Maine')) {
                stateDetails = `I\'ve detected that you currently have employees located in <strong>Portland, Maine</strong>, but do not currently have taxes configured for this state. Customers with similar setups have registered as an employer in the state before processing payroll.

To register as an employer in Maine customers typically complete the following:
• Register with <a href="https://www.maine.gov/revenue/taxes/tax-relief-credits-programs/income-tax-withholding" target="_blank" style="color: #0066cc; text-decoration: none;">Maine Revenue Services <svg style="width: 12px; height: 12px; display: inline-block; margin-left: 2px; vertical-align: baseline;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></a> for withholding
• Register with <a href="https://www.maine.gov/labor/unemployment/" target="_blank" style="color: #0066cc; text-decoration: none;">Maine Department of Labor <svg style="width: 12px; height: 12px; display: inline-block; margin-left: 2px; vertical-align: baseline;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></a> for SUI`;
            } else {
...
<strong>1. Add Maine taxes</strong>
2. I will <strong>upload the documents</strong> for extraction
3. These are <strong>remote employees</strong> from other states`;
            
            // No buttons for this scenario - user types their choice
            responseActions = [];
            break;
    }
...
You can add these details when configuring payment schedules, or your implementation specialist can help you enter them later.`, 'ai', [
            { action: 'proceed-to-payment-schedules', text: 'Continue to payment schedules', buttonType: 'primary' },
...
<strong>Recommendation:</strong> Most companies with remote Maine employees do register for Maine taxes to ensure compliance. You can add Maine taxes later if needed after consulting with your tax advisor.`, 'ai', [
            { action: 'proceed-to-payment-schedules', text: 'Continue without Maine taxes', buttonType: 'primary' },
        ...
        const summaryMessage = `Perfect! I\'ve successfully added all necessary taxes for your UKG migration:\n\n${newlyAddedTaxes.map(tax => `✅ ${tax.name}`).join('\n')}\n\nAll newly added taxes are marked with green "NEW" badges in the right panel. Would you like to continue with payment frequency setup?`;
        
        addMessage(summaryMessage, 'ai', [
            { action: 'assign-frequencies', text: 'Continue to payment frequencies', buttonType: 'primary' },
...
function addQuestionBelowTaxDisplay(questionText, responseActions) {
    ...
    if (taxConfigurationState.currentQuestion === 'business_expansion') {
        tierTwoOptions = [
            { action: 'maine-registration-time', text: 'How long does Maine registration take?' },
            { action: 'no-registration-consequence', text: 'What happens if I don\'t register before the next payroll?' }
        ];
    } else {
        tierTwoOptions = [
            { action: 'migration-info-1', text: 'What is UKG migration?' },
            { action: 'migration-info-2', text: 'Why are these taxes important?' },
            { action: 'migration-info-3', text: 'What if I\'m not sure?' }
        ];
    }
    ...
    if (migrationContext && Object.keys(migrationContext).length > 0) {
        ...
        setTimeout(() => {
            if (currentQuestion === 'unemployment_taxes' && employeeStates.length > foundStates.length) {
                ...
                // For business expansion with Maine, show confirmation then go to payment frequencies
                if ((migrationContext.stateHandling === 'business_expansion' || migrationContext.stateHandling === 'add_maine_taxes') && migrationContext.needsNewStateRegistrations) {
                    ...
                    console.log('Showing Maine expansion confirmation message');
                    addMessage(`Perfect! 5 state taxes for your Maine expansion are ready for configuration. How does everything look?`, 'assistant', [
                        { action: 'confirm-maine-taxes', text: 'Correct, move to payment frequency', buttonType: 'primary' },
                        { action: 'remove-maine-taxes', text: 'Remove Maine taxes', buttonType: 'secondary' }
                    ], {
                        style: 'two-tier-interactive',
                        tierTwoOptions: [
                            { action: 'configure-while-registering', text: 'I\'m still registering in Maine, can I configure taxes?' },
                            { action: 'tax-info-1', text: 'What is Maine PFML?' },
                            { action: 'tax-info-2', text: 'Do I need all 5 Maine taxes?' },
                            { action: 'tax-info-3', text: 'What are the Maine tax rates?' }
                        ]
                    });
                } else {
    ...
    if (migrationContext.needsUnemploymentRegistration || migrationContext.needsNewStateRegistrations) {
        ...
        if (taxCardsContainer && recommendations.toAdd.length > 0) {
            ...
            addMessage(`Great! I\'ve added ${recommendations.toAdd.length} new tax${recommendations.toAdd.length > 1 ? 'es' : ''} to your UKG setup:\n\n${recommendations.toAdd.map(tax => `✅ ${tax.name}`).join('\n')}\n\nThese have been marked as "Newly Added" in the right panel. Would you like to continue with the setup?`, 'ai', [
                ...
                style: 'two-tier-interactive',
                tierTwoOptions: [
                    { action: 'tax-info-1', text: 'What are payment frequencies?' },
...
function generateMigrationRecommendationMessage(recommendations) {
    ...
    message += '\n<strong>Next Step:</strong> Review the recommendations in the panel, then we\'ll configure payment frequencies for each tax.';
...
function skipRecommendations() {
    addMessage('No problem! You can always add these taxes later if needed.\n\nLet\'s continue with setting up payment frequencies for your confirmed taxes.', 'ai', [], {
        style: 'two-tier-interactive',
        tierTwoOptions: [
            { action: 'tax-info-1', text: 'What are payment frequencies?' },
...
function showFrequencyAssignment() {
    ...
    // Add user reply bubble first
    addMessage('Correct, move to payment frequency', 'user');
    ...
    setTimeout(() => {
        addMessage(`Now let's confirm your payment frequencies for MA and Maine. You can adjust these frequencies using the dropdowns.

<strong>Are the payment frequencies look right?</strong>`, 'ai', [
            { action: 'collect-tax-details', text: 'Frequencies look good', buttonType: 'primary' },
            { action: 'modify-frequencies', text: 'I need to change some', buttonType: 'secondary' }
        ], {
            style: 'two-tier-interactive',
            tierTwoOptions: [
                { action: 'frequency-help-1', text: 'What determines payment frequency?' },
...
function modifyFrequencies() {
    addMessage('Which payment frequency would you like to change?', 'ai', [
...
function showFrequencyHelp1() {
    addMessage('What determines payment frequency?', 'user');
    
    setTimeout(() => {
        addMessage(`<strong>Payment frequency is determined by:</strong>
...
function showFrequencyHelp2() {
    ...
    setTimeout(() => {
        addMessage(`<strong>Yes, you can change payment frequencies later!</strong>
...
function generateTaxMappingTable() {
    ...
    return `
        <div class="tax-mapping-table" style="padding: 20px;">
            ...
            <p style="color: #666; margin-bottom: 20px;">Review and confirm how these taxes will display on pay statements. Click any abbreviation to edit.</p>
...
function showPayStatementPreview() {
    ...
    setTimeout(() => {
        addMessage(`Great! Here's how taxes will appear on your employees' pay statements:
...
Please review the abbreviations in the table and confirm they match your preferences. You can edit any abbreviation by clicking on it.`, 'ai', [
            { action: 'abbreviations-confirmed', text: 'Abbreviations look correct', buttonType: 'primary' },
            { action: 'edit-abbreviations', text: 'I need to change some', buttonType: 'secondary' }
        ], {
            style: 'two-tier-interactive',
            tierTwoOptions: [
                { action: 'employee-questions-1', text: 'What will employees ask about?' },
                { action: 'employee-questions-2', text: 'How do I explain Maine taxes?' }
            ]
        });
    }, 500);
...
function handleAbbreviationsConfirmed() {
    addMessage('Abbreviations look correct', 'user');
    
    setTimeout(() => {
        addMessage(`Great! Your tax abbreviations are set up correctly.
...
• Massachusetts DUA rate assignment letter (for SUI rate verification)
• Maine Department of Labor rate notice (for new employer rates)` : 
`You'll need to provide:
• Massachusetts DUA rate assignment letter (for SUI rate verification)`}

These documents ensure your tax rates match what the state has assigned to your business.`, 'ai', [
            { action: 'configure-tax-rates', text: 'Noted, configure tax rates', buttonType: 'primary' },
...
function handleEditAbbreviations() {
    addMessage('I need to change some', 'user');
...
function configureTaxRates() {
    addMessage('Noted, configure tax rates', 'user');
    
    setTimeout(() => {
        // Update panel to show tax rate configuration
        updateTaxPanelContent('tax-rate-configuration');
        
        addMessage(`Let's configure your tax rates. I\'ll guide you through entering the exact rates from your state assignment letters.

<strong>We'll set up:</strong>
• Massachusetts SUI rate (from your DUA letter)
• Massachusetts EMAC rate 
• Federal unemployment rate (FUTA)
${taxConfigurationState.statesTaxes.some(s => s.state === 'Maine' && s.newlyAdded) ? 
'• Maine SUI rate (new employer rate)' : ''}

Having accurate rates ensures correct tax calculations from your first payroll.`, 'ai', [
            { action: 'start-rate-entry', text: 'Start entering rates', buttonType: 'primary' },
            { action: 'review-current-rates', text: 'Review current rates', buttonType: 'secondary' }
        ]);
    }, 800);
...
function showEmployeeQuestions1() {
    addMessage('What will employees ask about?', 'user');
...
function showEmployeeQuestions2() {
    addMessage('How do I explain Maine taxes?', 'user');
...
function generateFrequencyPanel() {
    ...
    return `
        <div class="frequency-assignment-panel">
            <h3>Payment Frequency Configuration</h3>
            <p style="color: #666; margin-bottom: 20px;">Configure how often you'll remit each tax</p>
                ...
                ${maineTaxes.length > 0 ? `
                    ...
                    <div class="tax-configuration-card" style="background: #ffffff; border: 2px solid #28a745; border-radius: 8px; padding: 20px; position: relative;">
                        ...
                        <div style="margin-top: 15px; padding: 12px; background: #e8f5e9; border-radius: 6px; border-left: 4px solid #28a745;">
                            <strong>Note:</strong> You'll need to register with Maine Revenue Services and Maine Department of Labor. We'll guide you through this process after configuration.
                        </div>
                    </div>
                ` : ''}
            ...
            <div style="margin-top: 30px; padding: 20px; background: #f0f8ff; border-radius: 8px; border-left: 4px solid #0066cc;">
                <h4 style="margin-top: 0; color: #0066cc;">Frequency Guidelines</h4>
                <ul style="margin: 10px 0; color: #555;">
                    <li><strong>Monthly:</strong> Most common for income tax withholding</li>
                    <li><strong>Quarterly:</strong> Typical for unemployment insurance (SUI/SUTA) and smaller employers</li>
                    <li><strong>Annual:</strong> Available for very small employers with minimal tax liability</li>
                </ul>
                <p style="margin-bottom: 0; color: #666; font-size: 14px;">
                    Your actual frequency may be assigned by the state based on your total tax liability.
                </p>
            </div>
        </div>
    `;
...
function confirmFrequencyAssignments() {
    ...
    addMessage(`Great! Your payment frequencies have been configured for Massachusetts and Maine taxes.
...
function updateTaxFrequency(taxId, frequency) {
    taxConfigurationState.paymentFrequencies[taxId] = frequency;
...
I've extracted state tax information from your uploaded tax documents. To ensure that I can accurately set these up for you, please review and make any necessary changes:
...
<strong>Are these state tax details correct?</strong>`, 'ai', [
            ...
            style: 'two-tier-interactive',
            tierTwoOptions: [
                { action: 'verify-state-registrations', text: 'How do I verify state registrations?' },
                { action: 'state-tax-requirements', text: 'What other state taxes are required?' },
                { action: 'wage-base-limit-enforcement', text: 'Do we automatically enforce wage base limit' }
            ],
            sourceDocuments: {
                count: 2,
                files: [
...
// Function to proceed to payment schedules after Maine decision
function proceedToPaymentSchedules() {
    addMessage('Continue to payment schedules', 'user');
...
function verifyStateRegistrations() {
    addMessage('How do I verify state registrations?', 'user');
    
    setTimeout(() => {
        addMessage(`<strong>Verifying State Tax Registrations:</strong>

<strong>State Tax Account Numbers to Locate:</strong>

<strong>New York:</strong>
• NYS Employer Registration Number
• Found on: Form NYS-45, NYS-100
• Verify at: NY Business Express website

<strong>New Jersey:</strong>  
• NJ Employer Registration Number
• Found on: Form NJ-927, REG-C-L
• Verify at: NJ Treasury MyNewJersey portal

<strong>Massachusetts:</strong>
• MA Withholding Account Number
• Found on: Form M-941, Tax ID letter
• Verify at: MassTaxConnect online

<strong>Documents needed for each state:</strong>
✓ State withholding account number
✓ Most recent state tax return
✓ State unemployment insurance number
✓ Workers' comp policy (if applicable)

<strong>Pro tip:</strong> Most states have online portals where you can verify your registration status and download account confirmations.`, 'ai', [
...
function addMaineTaxConfiguration() {
    ...
    setTimeout(() => {
        ...
        taxConfigurationState.statesTaxes.push({
            ...
            suiRate: '2.46', // 2025 new employer rate
            suiWageBase: '$12,000',
            sdiRate: 'N/A'
        });
...
Now let's move on to setting up your payment frequency schedules.`, 'ai');
        
        // Transition to payment frequency workflow after a brief pause
        setTimeout(() => {
            ...
            // Show payment schedule configuration with extracted data
            showPaymentScheduleConfiguration();
        }, 1500);
    }, 1000);
...
Now let's continue with setting up your payment frequency schedules.`, 'ai');
        
        // Continue to payment schedules after a brief pause
        setTimeout(() => {
            // Enter the pay-schedule workflow
            if (window.progressManager) {
                window.progressManager.enterWorkflow('pay-schedule', 0);
            ...
            // Show payment schedule configuration
            showPaymentScheduleConfiguration();
        }, 1500);
    }, 1000);
...
Once you\'ve made your changes, we can continue to payment frequency setup.`, 'ai', [
...
<strong>Pro tip:</strong> Setting up the configuration now means you're ready to go as soon as your registration is approved!`, 'ai', [
            { action: 'confirm-maine-taxes', text: 'Great, continue to payment frequency', buttonType: 'primary' },
...
Let's continue with setting up your payment frequencies.`, 'ai', [
            { action: 'confirm-maine-taxes', text: 'Continue to payment frequency', buttonType: 'primary' }
        ]);
    }, 800);
...
Let's continue with your Massachusetts tax payment frequencies.`, 'ai', [
            { action: 'proceed-to-payment-schedules', text: 'Continue to payment frequency', buttonType: 'primary' },
...
Let's continue with payment frequency setup.`, 'ai', [
            { action: 'confirm-maine-taxes', text: 'Continue to payment frequency', buttonType: 'primary' },
...
// Function to show payment schedule configuration with extracted data
function showPaymentScheduleConfiguration() {
    ...
    const extractedSchedules = [
        ...
        },
        {
            ...
            taxes: [
                ...
                { taxType: 'SUTA (Unemployment)', frequency: 'Quarterly' }, // Maine SUTA
    ...
    if (tablePanel && !tablePanel.querySelector('.state-tax-panel')) {
        // Generate payment schedule panel content only if tax panel isn't showing
        tablePanel.innerHTML = `
        <div class="payment-schedule-panel" style="padding: 24px; background: white; height: 100%; overflow-y: auto;">
...
Would you like to confirm these deposit schedules?`, 'ai', [
        { action: 'confirm-payment-schedules', text: 'Schedules look correct', buttonType: 'primary' },
        { action: 'modify-payment-schedules', text: 'Make adjustments', buttonType: 'secondary' }
    ], {
        style: 'two-tier-interactive',
        tierTwoOptions: [
            { action: 'what-is-payment-schedule', text: 'What is a payment schedule?' },
...
// Function to generate individual payment schedule card by state
...
// Handle payment schedule confirmation
function confirmPaymentSchedules() {
    ...
    setTimeout(() => {
        addMessage(`Excellent! Your payment schedules have been confirmed:

✓ Semi-Monthly Salary: 45 employees
✓ Weekly Hourly: 68 employees

The next step would typically be to configure the specific payment rules, holiday adjustments, and direct deposit settings for each schedule.

Your payroll configuration is taking shape nicely! The tax setup and payment schedules are now in place.`, 'ai', [
            { action: 'continue-to-next-setup', text: 'Continue to next setup', buttonType: 'primary' },
...
// Handle payment schedule modification
function modifyPaymentSchedules() {
    ...
    setTimeout(() => {
        addMessage(`Let's adjust your payment schedules. What would you like to modify?

You can:
• Add a new payment schedule for a different group
• Change the frequency of an existing schedule
• Adjust which employees are on which schedule
• Modify payment dates or rules
